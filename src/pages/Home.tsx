...
import { Play, ArrowRight, Users, BookOpen, Monitor, Award, CheckCircle2, Star, MapPin, GraduationCap, Sparkles, Brain, Target, Zap, ChevronRight } from 'lucide-react';
...
const CourseCard = ({ image, title, instructor, rating, location, duration, students, price, delay, onRequestQuery }: any) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
  ...
  >
    <div className="relative h-48 overflow-hidden">
      ...
      <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-bold text-gray-900 flex items-center gap-1">
        <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" /> {rating}
      </div>
    </div>
    <div className="p-5 flex flex-col flex-grow">
      <h3 className="font-bold text-lg text-gray-900 mb-4 line-clamp-2">{title}</h3>
      
      <div className="flex items-center gap-4 text-xs text-gray-500 mb-4 mt-auto">
        <div className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {location || duration || 'Location N/A'}</div>
        <div className="flex items-center gap-1"><Users className="w-4 h-4" /> {students}</div>
      </div>
      
      <div className="flex items-center justify-end pt-4 border-t border-gray-100">
        <button onClick={() => onRequestQuery(title)} className="px-4 py-2 bg-red-50 text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white rounded-lg text-sm font-medium transition-colors">
          Request Query
        </button>
      </div>
    </div>
      ...
      <section className="relative pt-20 pb-32 overflow-hidden bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 text-[var(--color-primary)] text-sm font-semibold mb-6">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[var(--color-primary)] opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-[var(--color-primary)]"></span>
                </span>
                New Live Batches Starting Today
              </div>
              ...
              <p className="text-lg text-gray-500 mb-8 max-w-xl leading-relaxed">
                Interactive live classes, recorded courses, AI learning, quizzes, assignments, and comprehensive progress tracking tailored for you.
              </p>
              
              <div className="flex flex-wrap gap-4">
                <Link to="/courses" className="px-8 py-4 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-white font-medium rounded-xl transition-all shadow-lg hover:shadow-red-200 hover:-translate-y-1 flex items-center gap-2">
            ...
            >
              ...
              <img 
                ...
                className="relative z-10 rounded-3xl shadow-2xl object-cover h-[80%] w-[90%]"
              />
              ...
              <motion.div 
                ...
                className="absolute top-10 -left-10 glass p-4 rounded-2xl shadow-xl flex items-center gap-4 z-20"
              >
              ...
              <motion.div 
                ...
                className="absolute top-1/2 -left-16 glass px-5 py-3 rounded-2xl shadow-xl flex items-center gap-3 z-20"
              >
      ...
      <section className="py-24 bg-white border-t border-gray-100 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full bg-red-50 text-[var(--color-primary)] text-xs sm:text-sm font-extrabold uppercase tracking-wider mb-4 border border-red-100">
              Academic Wings
            </span>
